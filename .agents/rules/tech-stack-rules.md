---
description: Fullstack Core Tech Stack & Development Rules with 5 Non-Negotiable Directives
globs: ["**/*"]
always_on: true
---
...
2. **সর্বোচ্চ পারফরম্যান্স অপ্টিমাইজেশন (High-Performance):** B-Tree/Composite/GIN ইনডেক্সিং, কার্সার পেজিনেশন, N+1 কোয়েরি প্রতিরোধ, মাল্টি-লেয়ার Redis ক্যাশিং, BullMQ অফলোডিং, Fastify, এবং RSC + Streaming।
...
- **Backend:** NestJS (Fastify Adapter) + Modular Architecture + PostgreSQL + Drizzle ORM + Redis + BullMQ + Passport.js + JWT (HTTP-Only Secure Cookies) + CASL + `nestjs-zod` + Swagger + Pino + Helmet + Throttler + Argon2 + Terminus.
- **DevOps & Infra:** Docker multi-stage Alpine + Docker Compose + Nginx + Turborepo + MinIO / R2 + Sentry + Uptime Kuma + GitHub Actions CI/CD.
