---
description: Fullstack Core Tech Stack & Rules (CloudLinux + LiteSpeed + cPanel + Firebase Auth & FCM)
globs: ["**/*"]
always_on: true
---
...
3. **নিশ্ছিদ্র সিকিউরিটি ও হ্যাকার আক্রমণ প্রতিরোধ (Bulletproof Security):** Firebase Managed Auth (পাসওয়ার্ড গুগলের সিকিউর ভল্টে থাকবে), OWASP Top 10 সুরক্ষা, Drizzle parameterized queries, Helmet, Throttler, Imunify360 ফ্রেন্ডলি কোড, এবং strictly `httpOnly` secure cookies।
...
## Core Tech Stack:
- **Frontend:** Next.js (App Router, Strict TS, Standalone output) + Firebase Modular SDK (`firebase/auth`, `firebase/messaging`) + RSC + Zustand + TanStack Query + React Hook Form + Zod + shadcn/ui + Radix UI + CSS Modules & CSS Variables + CVA + TanStack Virtual + Motion + next-intl (বাংলা ও ইংরেজি) + @react-pdf/renderer.
